...
class VAE(nn.Module):
    def __init__(self, win_len=100, h_dim=500, z_dim=20):
        super(VAE, self).__init__()
        self.win_len = win_len
        self.fc1 = nn.Linear(win_len, h_dim)
        self.fc2 = nn.Linear(h_dim, z_dim)
        self.fc3 = nn.Linear(h_dim, z_dim)
        self.fc4 = nn.Linear(z_dim, h_dim)
        self.fc5 = nn.Linear(h_dim, win_len)
...
def train(params, dataloader, device='cpu'):
    ...
    h_dim = params.get("h_dim")
    z_dim = params.get("z_dim")
    epoch_cnt = params.get("epoch_cnt")
    lr = params.get("lr")

    model = VAE(win_len=win_len, h_dim=h_dim, z_dim=z_dim).to(device)
    model.train()
    ...
    with torch.no_grad():
        for x, y in dataloader:
            if x.shape[0] != 1:
                ...
                raw_seq.append(x.cpu().numpy())
                est_seq.append(x_recons.cpu().numpy())
...
def online_test(model: VAE, train_dataloader, test_dataloader, device='cpu', threshold=0.99):
